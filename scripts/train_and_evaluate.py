...
from src.utils import load_dataset, split_dataset, MAX_EPOCHS, plot_training, calculate_accuracy, plot_confusion_matrix
...
def main() -> None:
    ...
    parser.add_argument(
        "--model-type", type=str, required=True, choices=["simple_heuristic", "neural_net", "knn", "random_forest"],
        help="Type of model "
             "to train. "
    )
    ...
    if model_type in ["knn", "random_forest"]:
        ...
        history = model.fit(train_X=train_X, train_y=train_y, test_X=test_X, test_y=test_y,
